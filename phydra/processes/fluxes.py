...
class Flux:
    ...
    growthrate_c1 = xs.variable(intent='in')
    growthrate_c1_allo = xs.variable(intent='out')
    ...
    def initialize(self):
        ...
        self.growthrate_c1_allo = np.linspace(self.growthrate_c1-0.1, self.growthrate_c1+0.1, self.gk_context['comp_dims'][self.c1_label])
        ...
        while not it1.finished:
            while not it2.finished:
                print(it1.multi_index)
                print(self.growthrate_c1_allo)
                self.gk_Fluxes[self.c1_label][it1.multi_index] = self.m.Intermediate(C1[it1.multi_index] * self.growthrate_c1_allo[it1.multi_index[1]])
                self.gk_Fluxes[self.c2_label][it2.multi_index] = self.m.Intermediate(C1[it1.multi_index] * self.conversion2_c2 - self.destruction_c2 * C2[it2.multi_index])
                it1.iternext()
                it2.iternext()